...
| Government warning checks, text and capitalization | Works: `backend/app/warning.py`. The comparison is exact; a difference of one or two characters is routed to human review with the character-level difference shown rather than reported as a mismatch, and is never a pass ([ADR 0012](docs/adr/0012-warning-near-miss.md)). A statement whose prefix OCR damaged is located by what survived, and a statement that differs only on lines the engine read badly is reported as present and not certified, a failing outcome that asks a person to look at the label ([ADR 0022](docs/adr/0022-warning-present-not-certified.md)). |
...
| Label artwork embedded in a COLA document | Works: every raster image above an area floor is lifted out of the PDF at its own resolution and read through the same OCR pipeline, largest first until the five values are in hand and up to a ceiling of eight, filling values the text layer left empty and, pooled, standing in as the label side when no photograph was uploaded; each value says which panel it was found on, and the response lists every embedded picture, read or set aside, with its size. The floor is an area alone since v1.5.0: a real filing whose labels are separate panels lost five of six pictures to the two shape rules that sat beside it (#121). Checking artwork from an application against that application is a self-consistency check, and the response and the interface both say so ([ADR 0010](docs/adr/0010-embedded-label-artwork.md)). It is read **once**, by the check: the prefill pass takes the document's text layer alone, and there is no cache ([ADR 0017](docs/adr/0017-read-the-artwork-once.md)). |
...
| COLA document parsing on real applications | **Measured on two real filings**, both committed unaltered as evidence in `samples/real/` so the measurement can be repeated ([ADR 0021](docs/adr/0021-real-filings-as-evidence-not-fixtures.md)): the mezcal returns five of five, and the bourbon found three defects, one of them the label's own. The item map is read off the blank TTB F 5100.31 (04/2023) and the three extraction paths are exercised in the suite against documents generated at test time; the two real documents are earlier editions of the printable Registry record, read by caption, and no test reads them. See OQ-22, closed, and A-17. |
