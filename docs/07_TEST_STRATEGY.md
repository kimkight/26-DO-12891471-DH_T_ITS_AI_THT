...
| Performance | Yes: single-label latency, a twelve-label batch of pairs and a twelve-label batch of filed applications alone, measured by the same script and by `test_batch.py` | A 300-label run, and any run on the deployed target |
...
| Warning legibility, `test_warning_legibility.py` | A statement whose prefix OCR damaged is located by what survived and its prefix reported unchecked; a letterless line is not part of it; a difference read below the floor on every differing line is present and not certified; an altered word in clean type, an omitted clause and a misspelt prefix in clean type are still mismatches; a near miss keeps its outcome; the plain-text path is unchanged (FR-5, FR-6, ADR 0022). |
...
| 53 | Open the batch view after doing all of the above | One control, no typing, no naming rule on screen; drop the filed applications alone and every one becomes a row, with the same chips beside the files and the same five cards behind each row as the single-label view | US-24; ADR 0009; ADR 0020 |
