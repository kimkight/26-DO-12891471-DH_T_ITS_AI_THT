...
| `TTB_MAX_DOCUMENT_READS` | `16` | The ceiling on how many Tesseract invocations reading one document may cost, its rendered pages and its embedded pictures together (NFR-1, ADR 0023; 24 until ADR 0025 took the orientation call out of the arithmetic, since a picture the document places makes none). Checked before each page and each picture, never inside one, so a picture is read whole; what it stops is listed as `not_reached` in the response, counted in `pages_not_reached`, and said in the notes. In reads rather than milliseconds so that the same document gets the same answer on every host; the bourbon filing's 19 reads are admitted with margin, and the runaway case of eight pictures at that filing's worst per-panel cost falls from 40 reads to 24 |
