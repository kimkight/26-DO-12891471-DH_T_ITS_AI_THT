...
| Requirements fully implemented | 14 of 21 |
| Requirements with an automated test | 14 of 21 |
| User stories | 21 |
...
NFR-2, NFR-3, NFR-6, NFR-7), is built and covered by 126 tests. What is not
built is what the agent would actually touch: there is no user interface, so
FR-10, NFR-4 and NFR-5 have nothing to test. Nothing is deployed.
...
page, is only half-satisfied by an API: the stream carries the position and the
total that make progress renderable, but nothing renders them until FR-10
exists.
