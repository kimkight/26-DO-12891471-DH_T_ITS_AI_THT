...
| NFR-1 About 5 seconds, **application-document path** | US-8, US-21 | #8, #21 | **No, missed at 6.8 s.** Measured on the deployed target 2026-08-30, build 1.1.0: the author's own mezcal COLA PDF submitted alone, three runs at 6883, 6786 and 6781 ms. The build under test also reported this as 3.2 s, because `elapsed_ms` measured the label-side OCR span rather than the request; that is fixed, `elapsed_ms` now covers the handler and a measured `timings` block is reported beside it. The duplicated OCR pass the honest measurement exposed is removed. **The 6.8 s figure stands until the same document is re-submitted to the deployed URL** | **Yes**: `test_timing.py` (13 tests: the recorder, the clock, the phases, the single read, per-row batch timing), `honestTiming.test.tsx` (8 tests) |
