...
| FR-5 Warning exact text, including the A-15 hyphenation rule and the ADR 0012 near-miss routing | US-4 | #4 | **Yes**: `app/warning.py`. The comparison is unchanged and still exact; a difference of at most `TTB_WARNING_NEAR_MISS_EDITS` characters is reported as needs human review with the character-level difference shown, and is never a pass. A capitalization failure is never a near miss | **Yes**: `test_warning.py::TestWarningBody`, `TestHyphenationAcrossLineBreaks` (unchanged in outcome), `test_warning_near_miss.py` (19 tests), `warningNearMiss.test.tsx` |
...
| NFR-1 About 5 seconds, **application-document path** | US-8, US-21 | #8, #21 | **Yes, met at about 3.5 s.** Re-measured on the deployed target 2026-08-30, deploy #11, build 1.1.0: the author's own mezcal COLA PDF (382 KB) submitted alone, three runs at 3468, 3505 and 3543 ms wall clock, with the response's own `elapsed_ms` at 3387, 3411 and 3463 ms and `unaccounted_ms` at 1.3 ms. `ocr_passes` read 1 against the 2 the same document paid before. This supersedes the 6883 / 6786 / 6781 ms recorded earlier the same day against the build that read the picture twice and reported only the OCR span as `elapsed_ms`. **One caveat, and it is not a rounding error: those runs were taken while the artwork OCR was still failing on this document, so the read that took 3.5 s was reading a wrongly turned, grayscale-flattened image. v1.1.0 changes what is read. The figure is re-measured after that lands and updated if it moves** (OQ-26) | **Yes**: `test_timing.py` (14 tests: the recorder, the clock, the phases, the single read, the engine invocations inside a pass, per-row batch timing), `honestTiming.test.tsx` (8 tests) |
