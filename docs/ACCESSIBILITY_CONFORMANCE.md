...
**Date of this report:** 2026-09-01
**Version evaluated:** v1.2.0, on `develop`, built by `npm run build` and served
from `dist/`, which is the page an agent loads.
...
| **A computed contrast test over the palette** | Every colour token against every surface it is used on, computed from `index.css` rather than from a table copied out of it. It checks pairs that no component happens to combine today, which is where a later change fails. | `frontend/src/__tests__/contrast.test.ts`, 49 assertions |
...
| 1.4.11 Non-text Contrast | AA (2.1) | Supports | The focus ring is checked at 3:1 against every ground it lands on: both surfaces, the pale navy pill track, the page shell, and the banner. The gold rules and edges are checked at 3:1 against every surface they sit on. The active pill is distinguished by weight, a shadow and `aria-selected` as well as by fill. |
...
| 2.4.4 Link Purpose (In Context) | A | Supports | The only links are the skip link and links within the Help tab's prose, each named by its own text. |
...
| 3.2.2 On Input | A | Supports | Typing into a field changes only that field's own source mark. Choosing a file starts a read and announces it; that is a change of content, not of context, and the agent asked for it. |
...
| Criterion | Level | Result | Evidence, and what was examined |
| --- | --- | --- | --- |
| 4.1.1 Parsing | A | Supports | The markup is generated by React and validated by the TypeScript build. axe's duplicate-id rules pass on every page; component-scoped ids come from `useId`. (This criterion is removed in WCAG 2.2 and is recorded here because Section 508 adopts 2.0.) |
...
| 4.1.3 Status Messages | AA (2.1) | Supports | Four polite `role="status"` regions, each labelled apart so a screen reader says which one spoke: the uploads, the application values, the check result, and the batch progress. Each is always in the DOM rather than mounted with its text, because a live region added at the same moment as its content is not reliably announced. The reset's announcement uses the check-result region, which is the one it emptied. |
