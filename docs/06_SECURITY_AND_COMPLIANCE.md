...
| `id-token: write` on the two jobs that assume the AWS role, and on no other | `.github/workflows/deploy.yml` | The workflow default is `contents: read`; the SBOM-attaching job holds `contents: write` and no AWS session; the build job holds `actions: read` as well, which `anchore/sbom-action` needs on a release event to list workflow runs, and which no other job holds (v1.3.1) |
